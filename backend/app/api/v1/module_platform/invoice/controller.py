...
async def invoice_apply_controller(
    ...
    result = await InvoiceTenantService.apply(auth, data, auth.user.tenant_id if auth.user else 0)
...
async def invoice_download_controller(
    ...
    pdf_url = await InvoiceTenantService.download(auth, id, auth.user.tenant_id if auth.user else 0)
