...
async def update_tenant_config_controller(
    ...
    result = await TenantService(auth).update_config(redis=redis, tenant_id=id, config=config_dict)
    await FastAPICache.clear(namespace=_TENANT_NS)
...
async def renew_tenant_controller(
    ...
    result = await TenantService(auth).renew(tenant_id=id, end_time=end_time_str)
    await FastAPICache.clear(namespace=_TENANT_NS)
...
async def package_available_controller(
    ...
    result = await TenantService.get_available_packages(auth=auth, tenant_id=auth.user.tenant_id if auth.user else 0)
...
async def package_preview_controller(
    ...
    result = await TenantService.preview_package_change(auth=auth, tenant_id=auth.user.tenant_id if auth.user else 0, target_package_id=target_package_id)
    return SuccessResponse(data=result, msg="查询成功")
...
async def order_create_controller(
    ...
    result = await TenantService.create_self_order(auth=auth, tenant_id=auth.user.tenant_id if auth.user else 0, data=data)
...
async def tenant_workspace_controller(
    ...
    result = await TenantService.get_workspace_data(auth=auth, tenant_id=auth.user.tenant_id if auth.user else 0)
    return SuccessResponse(data=result, msg="查询成功")
