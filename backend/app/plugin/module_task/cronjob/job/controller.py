...
@JobRouter.get("/scheduler/status", summary="获取调度器状态", response_model=ResponseSchema[dict], dependencies=[Depends(AuthPermission(["module_task:cronjob:job:query"]))])
...
@JobRouter.get("/scheduler/jobs", summary="获取调度器任务列表", response_model=ResponseSchema[list[dict]], dependencies=[Depends(AuthPermission(["module_task:cronjob:job:query"]))])
...
@JobRouter.post("/scheduler/start", summary="启动调度器", response_model=ResponseSchema[None], dependencies=[Depends(AuthPermission(["module_task:cronjob:job:scheduler"]))])
async def start_scheduler_controller() -> JSONResponse:
    SchedulerUtil.start()
    return SuccessResponse(msg="调度器已启动")


@JobRouter.post("/scheduler/pause", summary="暂停调度器", response_model=ResponseSchema[None], dependencies=[Depends(AuthPermission(["module_task:cronjob:job:scheduler"]))])
async def pause_scheduler_controller() -> JSONResponse:
    SchedulerUtil.pause()
    return SuccessResponse(msg="调度器已暂停")


@JobRouter.post("/scheduler/resume", summary="恢复调度器", response_model=ResponseSchema[None], dependencies=[Depends(AuthPermission(["module_task:cronjob:job:scheduler"]))])
async def resume_scheduler_controller() -> JSONResponse:
    SchedulerUtil.resume()
    return SuccessResponse(msg="调度器已恢复")


@JobRouter.post("/scheduler/shutdown", summary="关闭调度器", response_model=ResponseSchema[None], dependencies=[Depends(AuthPermission(["module_task:cronjob:job:scheduler"]))])
async def shutdown_scheduler_controller() -> JSONResponse:
    await SchedulerUtil.shutdown()
    return SuccessResponse(msg="调度器已关闭")


@JobRouter.delete("/scheduler/jobs/clear", summary="清空所有任务", response_model=ResponseSchema[None], dependencies=[Depends(AuthPermission(["module_task:cronjob:job:task"]))])
async def clear_jobs_controller() -> JSONResponse:
    SchedulerUtil.clear_jobs()
    return SuccessResponse(msg="已清空所有任务")


@JobRouter.get("/scheduler/console", summary="获取调度器控制台信息", response_model=ResponseSchema[str], dependencies=[Depends(AuthPermission(["module_task:cronjob:job:query"]))])
...
@JobRouter.post("/scheduler/sync", summary="同步调度器任务到数据库", response_model=ResponseSchema[int], dependencies=[Depends(AuthPermission(["module_task:cronjob:job:update"]))])
...
@JobRouter.post("/task/pause/{job_id}", summary="暂停任务", response_model=ResponseSchema[None], dependencies=[Depends(AuthPermission(["module_task:cronjob:job:task"]))])
...
@JobRouter.post("/task/resume/{job_id}", summary="恢复任务", response_model=ResponseSchema[None], dependencies=[Depends(AuthPermission(["module_task:cronjob:job:task"]))])
...
@JobRouter.post("/task/run/{job_id}", summary="立即执行任务", response_model=ResponseSchema[None], dependencies=[Depends(AuthPermission(["module_task:cronjob:job:task"]))])
...
@JobRouter.delete("/task/remove/{job_id}", summary="移除任务", response_model=ResponseSchema[None], dependencies=[Depends(AuthPermission(["module_task:cronjob:job:delete"]))])
