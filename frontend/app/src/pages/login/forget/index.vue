...
const forgetSchema: FormSchema = {
  validate: (model) => {
    ...
    if (!username)
      errors.push({ path: ['username'], message: t('forget.usernameRequired') })
    else if (!USERNAME_REG.test(username))
      errors.push({ path: ['username'], message: t('forget.usernameFormat') })
    if (!password)
      errors.push({ path: ['new_password'], message: t('forget.passwordRequired') })
    else if (password.length < 6)
      errors.push({ path: ['new_password'], message: t('forget.passwordLength') })
    if (!confirmPassword)
      errors.push({ path: ['confirmPassword'], message: t('forget.confirmRequired') })
    else if (confirmPassword !== password)
      errors.push({ path: ['confirmPassword'], message: t('forget.mismatch') })
    return errors
  },
}
        ...
        <wd-form ref="forgetFormRef" :model="forgetForm" :schema="forgetSchema">
          <wd-form-item prop="username">
            <wd-input
              ...
              :placeholder="$t('forget.usernamePlaceholder')"
              clearable
              :compact="false"
              prefix-icon="user"
            />
          ...
          <wd-form-item prop="confirmPassword">
            <wd-input
              ...
              :placeholder="$t('forget.confirmPlaceholder')"
              show-password
              clearable
              :compact="false"
              prefix-icon="lock"
