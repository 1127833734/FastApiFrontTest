...
const registerSchema: FormSchema = {
  validate: (model) => {
    ...
    if (!username)
      errors.push({ path: ['username'], message: t('register.usernameRequired') })
    else if (!USERNAME_REG.test(username))
      errors.push({ path: ['username'], message: t('register.usernameFormat') })
    if (!name)
      errors.push({ path: ['name'], message: t('register.nicknameRequired') })
    if (!password)
      errors.push({ path: ['password'], message: t('register.passwordRequired') })
    else if (password.length < 6)
      errors.push({ path: ['password'], message: t('register.passwordLength') })
    if (!confirmPassword)
      errors.push({ path: ['confirmPassword'], message: t('register.confirmRequired') })
    else if (confirmPassword !== password)
      errors.push({ path: ['confirmPassword'], message: t('register.mismatch') })
    return errors
  },
}
