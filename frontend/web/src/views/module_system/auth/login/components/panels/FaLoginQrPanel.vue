...
<template>
  <div>
    <div class="login-page-form login-qr-flow flex w-full min-w-0 flex-col items-center">
      <div
        class="login-qr-card flex w-full max-w-70 flex-col items-center rounded-2xl bg-white p-6 shadow-[0_8px_24px_rgb(0_0_0_/12%)] dark:shadow-[0_8px_28px_rgb(0_0_0_/45%)]"
      >
        <QrcodeVue class="block" :value="qrPayload" v-bind="qrCodeRenderOptions" />
      </div>

      <p
        class="login-qr-hint mt-4 max-w-70 text-center text-sm leading-relaxed text-(--el-text-color-secondary)"
      >
        {{ $t("login.qrLoginHint") }}
      </p>
